    ...
    try {
      ...
      if (exchangeError) {
        ...
        const errorResponse = NextResponse.redirect(
          `${requestUrl.origin}/login?error=auth_exchange_failed`,
        );
      ...
      if (sessionError || !session) {
        ...
        const errorResponse = NextResponse.redirect(
          `${requestUrl.origin}/login?error=session_fetch_failed`,
        );
        return errorResponse;
      }
      ...
      const errorResponse = NextResponse.redirect(
        `${requestUrl.origin}/login?error=callback_unexpected_error`,
      );
      return errorResponse;
    }
  }
