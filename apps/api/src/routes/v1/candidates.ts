...
const PersonalInfoSchema = z
  .object({
    name: z.string().describe('Full name of the candidate').openapi({ example: 'Sarah Johnson' }),
    email: z.string().email().optional().describe('Email address').openapi({ example: 'sarah.johnson@email.com' }),
    phone: z.string().optional().describe('Phone number').openapi({ example: '+1-555-0123' }),
    location: z.string().optional().describe('Current location').openapi({ example: 'San Francisco, CA' }),
    linkedinUrl: z
      .string()
      .url()
      .optional()
...
const ExperienceSchema = z
  .object({
    company: z.string().describe('Company name').openapi({ example: 'Tech Unicorn Inc' }),
    position: z.string().describe('Job title/position').openapi({ example: 'Senior Software Engineer' }),
    startDate: z.string().describe('Start date (YYYY-MM format)').openapi({ example: '2021-03' }),
    endDate: z
      .string()
      .optional()
      .describe('End date (YYYY-MM format), null if current position')
      .openapi({ example: '2024-01' }),
    description: z.string().describe('Job description and responsibilities').openapi({
      example:
    ...
    location: z.string().optional().describe('Job location').openapi({ example: 'San Francisco, CA' }),
  })
  .describe('Work experience entry')

const EducationSchema = z
  .object({
    institution: z.string().describe('Educational institution name').openapi({ example: 'Stanford University' }),
    degree: z.string().describe('Degree type').openapi({ example: 'Bachelor of Science' }),
    field: z.string().describe('Field of study').openapi({ example: 'Computer Science' }),
    graduationDate: z.string().optional().describe('Graduation date (YYYY-MM format)').openapi({ example: '2018-06' }),
    gpa: z.string().optional().describe('Grade point average').openapi({ example: '3.8' }),
  })
  .describe('Education entry')

const ProjectSchema = z
  .object({
    name: z.string().describe('Project name').openapi({ example: 'E-commerce Analytics Dashboard' }),
    description: z
      .string()
      .describe('Project description')
      .openapi({ example: 'Real-time analytics dashboard for e-commerce platforms' }),
    technologies: z
      .array(z.string())
      .describe('Technologies used')
      .openapi({ example: ['TypeScript', 'D3.js', 'Express.js', 'Redis'] }),
    url: z.string().url().optional().describe('Live project URL').openapi({ example: 'https://analytics.example.com' }),
    githubUrl: z
      .string()
      .url()
      .optional()
      .describe('GitHub repository URL')
      .openapi({ example: 'https://github.com/sarahjohnson/analytics-dashboard' }),
  })
  .describe('Project entry')

const CertificationSchema = z
  .object({
    name: z.string().describe('Certification name').openapi({ example: 'AWS Solutions Architect Associate' }),
    issuer: z.string().describe('Issuing organization').openapi({ example: 'Amazon Web Services' }),
    issueDate: z.string().optional().describe('Issue date (YYYY-MM format)').openapi({ example: '2023-05' }),
    expirationDate: z.string().optional().describe('Expiration date (YYYY-MM format)').openapi({ example: '2026-05' }),
    credentialId: z
      .string()
      .optional()
...
const ParsedResumeDataSchema = z
  .object({
    ...
    summary: z.string().optional().describe('Professional summary or objective').openapi({
      example:
    ...
    skills: z
      ...
      .openapi({
        example: ['React', 'TypeScript', 'Node.js', 'AWS', 'Docker', 'PostgreSQL'],
      }),
    projects: z.array(ProjectSchema).describe('Notable projects and portfolio items'),
    certifications: z.array(CertificationSchema).describe('Professional certifications'),
    languages: z
      .array(z.string())
...
const SkillMatchSchema = z
  .object({
    skill: z.string().describe('Skill or technology name').openapi({ example: 'React' }),
    found: z.boolean().describe("Whether this skill was found in candidate's resume").openapi({ example: true }),
    confidence: z.number().min(0).max(1).describe('Confidence score for skill match (0-1)').openapi({ example: 0.95 }),
    context: z
      .string()
      .optional()
...
const CandidateScoreSchema = z
  .object({
    candidateId: z.string().describe('Candidate unique identifier').openapi({ example: 'candidate-123' }),
    jobId: z.string().describe('Job posting unique identifier').openapi({ example: 'job-456' }),
    overallScore: z.number().min(0).max(100).describe('Overall candidate fit score (0-100)').openapi({ example: 85 }),
    requiredSkillsScore: z
      ...
      .openapi({ example: 90 }),
    experienceScore: z.number().min(0).max(100).describe('Experience relevance score (0-100)').openapi({ example: 80 }),
    educationScore: z.number().min(0).max(100).describe('Education background score (0-100)').openapi({ example: 85 }),
    skillMatches: z.array(SkillMatchSchema).describe('Detailed skill matching results'),
    ...
    recommendations: z
      ...
      .openapi({
        example: ['🌟 Excellent candidate - highly recommended for interview', '💡 Has all required technical skills'],
      }),
  })
...
const ParseResumeRequestSchema = z
  .object({
    candidateId: z.string().describe('Unique identifier for the candidate').openapi({ example: 'candidate-123' }),
    jobId: z.string().describe('Job posting ID to match against for skill scoring').openapi({ example: 'job-456' }),
    fileContent: z.string().describe('Resume text content (extracted from PDF/DOCX/TXT files)').openapi({
      example:
...
const ParseResumeResponseSchema = z
  .object({
    ...
    data: z
      .object({
        parsedData: ParsedResumeDataSchema.describe('Structured resume data extracted by AI'),
        score: CandidateScoreSchema.describe('Candidate scoring and skill matching results'),
      })
      .describe('Response payload containing parsed resume and scoring data'),
    metadata: MetadataSchema.describe('Request metadata including processing time and correlation ID'),
  })
...
candidatesRoutes.openapi(parseResumeRoute, async (c) => {
  ...
  try {
    ...
    const parsedData = await aiService.parseResume(body.fileContent, body.fileName)
    ...
    const jobRequirements = await jobRequirementsService.getJobRequirements(body.jobId)
    ...
    const baseScore = skillMatcher.calculateCandidateScore(parsedData, jobRequirements)

    const recommendations = skillMatcher.generateRecommendations(parsedData, baseScore, jobRequirements)

    const score = { ...baseScore, candidateId, recommendations }

    const shouldAutoReject = skillMatcher.shouldAutoReject(score, jobRequirements)
    ...
    try {
      ...
      logger.warn('Failed to persist score to database (continuing with response)', {
        error: dbError,
        candidateId,
        jobId: body.jobId,
