...
const jobPostingSchema = z.object({
	...
	job_type: z
		.string()
		.describe("Job type (full-time, part-time, contract, etc.)"),
	...
	location_display: z
		.string()
		.nullable()
		.describe("Location as displayed to candidates"),
	location_type: z
		.string()
		.nullable()
		.describe("Location type (remote, office, hybrid)"),
	remote_allowed: z
		.boolean()
		.nullable()
		.describe("Whether remote work is allowed"),
	salary_min: z.number().nullable().describe("Minimum salary range"),
	salary_max: z.number().nullable().describe("Maximum salary range"),
	salary_type: z
		.string()
		.nullable()
		.describe("Salary type (hourly, annual, etc.)"),
	pipeline_template_id: z
		.string()
		.uuid()
		.nullable()
		.describe("Associated pipeline template ID"),
	form_template_id: z
		.string()
		.uuid()
		.nullable()
		.describe("Associated form template ID"),
...
const createJobSchema = z.object({
	...
	content: z
		.record(z.any())
		.describe(
			"Job posting content (description, requirements, benefits, etc.)",
		),
	status: z
	...
	salary_type: z
		.enum(["hourly", "daily", "salary"])
		.optional()
		.describe("Salary payment frequency"),
	pipeline_template_id: z
		.string()
		.uuid()
		.optional()
		.describe("Pipeline template to use for this job"),
	form_template_id: z
		.string()
		.uuid()
		.optional()
		.describe("Application form template to use"),
});
...
const listJobsRoute = createRoute({
	...
	description:
		"Retrieve all job postings for your organization with optional filtering and pagination",
	security: [{ apiKey: [] }],
	request: {
		query: z.object({
			status: z
...
publicJobManagementRoutes.openapi(listJobsRoute, async (c: any) => {
	...
	try {
		const apiKeyMeta = c.get("apiKeyMeta") as {
			companyId: string;
			tier: string;
			permissions: string[];
		};
