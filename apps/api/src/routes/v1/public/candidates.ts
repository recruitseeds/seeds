...
const PersonalInfoSchema = z.object({
  ...
  portfolioUrl: z.string().url().optional().describe('Personal portfolio website'),
})
...
const CandidateScoreSchema = z.object({
  ...
  requiredSkillsScore: z.number().min(0).max(100).describe('Score for required skills match (0-100)'),
  experienceScore: z.number().min(0).max(100).describe('Experience relevance score (0-100)'),
  educationScore: z.number().min(0).max(100).describe('Education background score (0-100)'),
...
const ParseResumeRequestSchema = z.object({
  body: z.object({
    ...
    fileContent: z.string().min(10, 'Resume content must be at least 10 characters'),
...
const parseResumeRoute = createRoute({
  ...
  request: {
    params: z.object({
      id: z.string().describe('Candidate ID - must match candidateId in request body'),
    }),
    body: {
      content: {
        'application/json': {
          schema: z.object({
            ...
            jobId: z.string().uuid().describe('Job posting ID to match against for skill scoring'),
            fileContent: z.string().min(10).describe('Resume text content (extracted from PDF/DOCX/TXT)'),
            fileName: z.string().min(1).describe('Original filename for processing context'),
          }),
        },
      },
    },
  ...
  responses: {
    200: {
      ...
      description: 'Resume parsed successfully with comprehensive skill matching analysis',
    },
    400: {
      content: {
        'application/json': {
...
const validateCandidateOwnership = async (c: Context) => {
  ...
  if (candidateId !== c.req.param('id')) {
    throw new Error('Candidate ID in URL must match candidateId in request body')
  }
}
...
publicCandidatesRoutes.openapi(
  ...
  async (c) => {
    ...
    try {
      ...
      const jobRequirements = await jobRequirementsService.getJobRequirements(jobId)
      const baseScore = skillMatcherService.calculateCandidateScore(parsedData, jobRequirements)
      const recommendations = skillMatcherService.generateRecommendations(parsedData, baseScore, jobRequirements)
      ...
      try {
        await candidateScoringService.saveScore(score, {
          ...
          autoRejected: skillMatcherService.shouldAutoReject(score, jobRequirements),
        })
        logger.info('Score saved to database', { candidateId, jobId, overallScore: score.overallScore })
      } catch (dbError) {
        logger.warn('Failed to save score (continuing)', { error: dbError, candidateId })
      }
...
const getScoreRoute = createRoute({
  ...
  request: {
    ...
    query: z.object({
      ...
      includeDetails: z.enum(['true', 'false']).optional().describe('Include detailed breakdown (requires additional permissions)'),
    }),
  },
  responses: {
    200: {
...
const validateScoreAccess = async (c: Context) => {
  ...
  if (c.get('validatedData').query.includeDetails === 'true' && !permissions.includes('scores:read:detailed')) {
    logger.warn('Detailed score access denied - insufficient permissions', {
      candidateId,
      companyId,
      requiredPermission: 'scores:read:detailed',
    })
    throw new Error('Detailed score access requires scores:read:detailed permission')
  }
}
