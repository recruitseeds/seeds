...
import { EmailService, CandidateApplicationEmailData } from '../../src/services/email.js'
...
describe('EmailService', () => {
  ...
  describe('sendEmail', () => {
    it('should send email successfully', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Sending email', expect.any(Object))
      expect(mockLogger.info).toHaveBeenCalledWith('Email sent successfully', expect.any(Object))
    })
    ...
    it('should handle Resend API errors', async () => {
      ...
      await expect(emailService.sendEmail(emailData)).rejects.toThrow('Email sending failed: Invalid API key')
      expect(mockLogger.error).toHaveBeenCalledWith('Email sending failed', expect.any(Object), expect.any(Object))
    })
    ...
    it('should handle unexpected errors', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Unexpected error sending email', expect.any(Error), expect.any(Object))
    })
    ...
    it('should include metadata in logs', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Sending email', expect.objectContaining({
        correlationId: 'corr-123',
        candidateId: 'candidate-456',
        companyId: 'company-789'
      }))
  ...
  describe('sendTemplatedEmail', () => {
    ...
    it('should handle conditional template sections', async () => {
      ...
      expect(sentEmail.text).not.toContain('You can track your application status at:')
    })
  })
  ...
  describe('sendApplicationReceivedEmail', () => {
    it('should send application received email with all data', async () => {
      ...
      expect(result).toBe('email-123')
      expect(mockLogger.info).toHaveBeenCalledWith('Sending application received email', expect.objectContaining({
        candidateEmail: 'alice@example.com',
        jobTitle: 'Senior Engineer',
        companyName: 'TechStartup',
        applicationId: 'app-789',
      ...
      expect(sentEmail.html).toContain('https://portal.techstartup.com/app-789')
  ...
  describe('template management', () => {
    it('should return template by ID', () => {
      ...
      expect(template?.requiredVariables).toEqual(['candidateName', 'jobTitle', 'companyName', 'applicationId'])
    })
