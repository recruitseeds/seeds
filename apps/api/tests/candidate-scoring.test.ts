...
describe('CandidateScoringService', () => {
  ...
  it('should save and retrieve candidate score', async () => {
    const scoreId = await candidateScoringService.saveScore(testScore, testMetadata)
  ...
  it('should handle auto-rejected candidates', async () => {
    ...
    const scoreId = await candidateScoringService.saveScore(autoRejectedScore, autoRejectedMetadata)
    expect(scoreId).toBeDefined()
  ...
  it('should get candidate scores for a specific candidate', async () => {
    ...
    const candidateScores = await candidateScoringService.getCandidateScores(candidateId)
    
    expect(candidateScores).toHaveLength(3)
    expect(candidateScores.map(s => s.overallScore).sort()).toEqual([78, 85, 92])
  })
  ...
  it('should get job scores with filtering', async () => {
    ...
    const highScorers = await candidateScoringService.getJobScores(jobId, { minScore: 85 })
    expect(highScorers.length).toBeGreaterThanOrEqual(2)
    expect(highScorers.every(c => c.overallScore >= 85)).toBe(true)
    
    expect(highScorers[0].overallScore).toBeGreaterThanOrEqual(highScorers[1]?.overallScore || 0)
  })

  it('should return null for non-existent score', async () => {
    const result = await candidateScoringService.getScore('non-existent-candidate', 'non-existent-job')
