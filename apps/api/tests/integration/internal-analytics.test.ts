  ...
  createClient: vi.fn().mockImplementation(() => ({
    from: vi.fn().mockImplementation((table: string) => ({
      ...
      limit: vi.fn().mockResolvedValue({
        data: [
          { overall_score: 85, required_skills_score: 90, experience_score: 80, processing_time_ms: 2500 },
          { overall_score: 75, required_skills_score: 80, experience_score: 85, processing_time_ms: 3000 }
        ],
...
describe('Internal Analytics API', () => {
  ...
  describe('GET /internal/analytics/dashboard', () => {
    ...
    it('should handle database errors gracefully', async () => {
      vi.mocked(require('@supabase/supabase-js').createClient).mockImplementationOnce(() => ({
