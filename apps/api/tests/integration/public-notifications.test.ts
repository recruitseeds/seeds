...
describe('Public Notifications API', () => {
  ...
  describe('POST /public/notifications/application-received', () => {
    ...
    it('should send application confirmation email successfully', async () => {
      ...
      const response = await app.request('/public/notifications/application-received', {
        method: 'POST',
        headers: {
    ...
    it('should validate UUID formats', async () => {
      ...
      expect(body.success).toBe(false)
      expect(body.error.code).toBe('VALIDATION_ERROR')
      expect(body.error.details.some((detail: any) => detail.message.includes('Must be a valid UUID'))).toBe(true)
    })
    ...
    it('should validate email addresses', async () => {
      ...
      expect(body.success).toBe(false)
      expect(body.error.code).toBe('VALIDATION_ERROR')
      expect(body.error.details.some((detail: any) => detail.message.includes('Must be a valid email'))).toBe(true)
    })
    ...
    it('should handle email service errors gracefully', async () => {
      mockSendApplicationReceivedEmail.mockRejectedValue(new Error('Resend API error'))
    ...
    it('should handle unexpected errors', async () => {
      mockSendApplicationReceivedEmail.mockRejectedValue(new Error('Unexpected database error'))
      ...
      expect(body.error.message).toBe('An error occurred while sending the email')
    })
