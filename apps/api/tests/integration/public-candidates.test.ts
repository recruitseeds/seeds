...
vi.mock('../../src/services/skill-matcher.js', () => ({
  SkillMatcherService: vi.fn().mockImplementation(() => ({
    calculateCandidateScore: vi.fn().mockReturnValue({
      ...
      educationScore: 85,
      skillMatches: [
        { skill: 'JavaScript', found: true, confidence: 1.0, context: 'Found exact match' },
        { skill: 'React', found: true, confidence: 1.0, context: 'Found exact match' },
        { skill: 'Node.js', found: true, confidence: 1.0, context: 'Found exact match' }
      ],
...
describe('Public Candidates API', () => {
  ...
  describe('POST /public/candidates/:id/parse-resume', () => {
    it('should successfully parse resume with valid authentication', async () => {
      ...
      const response = await app.request('/public/candidates/candidate-123/parse-resume', {
        method: 'POST',
        headers: {
    ...
    it('should handle AI service errors gracefully', async () => {
      vi.mocked(require('../../src/services/ai.js').AIService).mockImplementationOnce(() => ({
  ...
  describe('GET /public/candidates/:id/score', () => {
    beforeEach(() => {
      vi.mocked(require('../../src/services/candidate-scoring.js').CandidateScoringService).mockReset()
      vi.mocked(require('../../src/services/candidate-scoring.js').CandidateScoringService).mockImplementation(() => ({
        saveScore: vi.fn().mockResolvedValue('test-score-id'),
    ...
    it('should successfully retrieve score with valid authentication', async () => {
      const response = await app.request('/public/candidates/candidate-123/score?jobId=job-456', {
        method: 'GET',
        headers: {
          'Authorization': 'Bearer uk_test_key_123'
        }
        ...
        Promise.resolve({
          ...
          json: () => Promise.resolve({
            ...
            meta: {
              tier: 'pro',
              companyId: 'company-123',
              permissions: ['candidates:read', 'candidates:write', 'scores:read:detailed']
            }
          })
        } as any)
      )

      const response = await app.request('/public/candidates/candidate-123/score?jobId=job-456&includeDetails=true', {
        method: 'GET',
        headers: {
          'Authorization': 'Bearer uk_test_key_123'
        }
    ...
    it('should return 404 when score not found', async () => {
      vi.mocked(require('../../src/services/candidate-scoring.js').CandidateScoringService).mockImplementationOnce(() => ({
    ...
    it('should validate required query parameters', async () => {
      const response = await app.request('/public/candidates/candidate-123/score', {
        method: 'GET',
        headers: {
          'Authorization': 'Bearer uk_test_key_123'
        }
    ...
    it('should validate UUID format for candidate ID', async () => {
      const response = await app.request('/public/candidates/invalid-id/score?jobId=job-456', {
        method: 'GET',
        headers: {
          'Authorization': 'Bearer uk_test_key_123'
        }
    ...
    it('should validate UUID format for job ID', async () => {
      const response = await app.request('/public/candidates/candidate-123/score?jobId=invalid-job-id', {
        method: 'GET',
        headers: {
          'Authorization': 'Bearer uk_test_key_123'
        }
    ...
    it('should handle database errors gracefully', async () => {
      vi.mocked(require('../../src/services/candidate-scoring.js').CandidateScoringService).mockImplementationOnce(() => ({
        getScore: vi.fn().mockRejectedValue(new Error('Database connection failed'))
      }))
      ...
      expect(body.error.message).toContain('Failed to retrieve candidate score')
    })

    it('should only accept true/false values for includeDetails', async () => {
      const response = await app.request('/public/candidates/candidate-123/score?jobId=job-456&includeDetails=maybe', {
        method: 'GET',
        headers: {
          'Authorization': 'Bearer uk_test_key_123'
        }
