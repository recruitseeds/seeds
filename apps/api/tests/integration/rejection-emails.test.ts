...
describe('Rejection Email System', () => {
  ...
  afterAll(async () => {
    ...
    await supabase.from('organization_rejection_settings').delete().neq('organization_id', '')
  })
  ...
  describe('schedule_rejection_email function', () => {
    ...
    it('should respect business days setting', async () => {
      ...
      const daysDiff = Math.ceil((scheduledTime.getTime() - startTime.getTime()) / (1000 * 60 * 60 * 24))
  ...
  describe('get_pending_rejection_emails function', () => {
    it('should return emails that are due', async () => {
      ...
      const { data, error } = await supabase.rpc('get_pending_rejection_emails')
      ...
      const email = data!.find((e: any) => e.application_id === testApplicationId)
  ...
  describe('POST /api/v1/internal/cron/send-rejection-emails', () => {
    it('should require internal authentication', async () => {
      const response = await app.request('/api/v1/internal/cron/send-rejection-emails', {
        method: 'POST',
        headers: {
  ...
  describe('mark_rejection_email_sent function', () => {
    it('should mark email as sent and update application status', async () => {
      const { data: emailData } = await supabase.from('scheduled_rejection_emails').insert({
        application_id: testApplicationId,
        recipient_email: 'test@example.com',
