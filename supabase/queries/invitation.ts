...
export async function getInvitationsByOrganization(
  ...
  let query = supabase
    ...
    .select(`
      *,
      invited_by:organization_users!organization_invitations_invited_by_fkey(
        name
      ),
  ...
  const enrichedData = data?.map((invitation) => ({
    ...invitation,
    inviter_name: (invitation.invited_by as { name?: string })?.name || 'Unknown',
    organization_name: (invitation.organization as { name?: string })?.name || 'Unknown Organization',
  }))
...
export async function getInvitationByToken(
  ...
  const enrichedData = {
    ...
    organization_name: (data.organization as { name?: string })?.name || 'Unknown Organization',
  }
