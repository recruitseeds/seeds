...
import {
  ...
  DropdownMenuItem,
  DropdownMenuTrigger,
} from '@/components/ui/editor/dropdown-menu'
import {
  HeadingButton,
  type Level,
  ShortcutKey,
  ...
  headingIcons,
} from '@/components/ui/editor/heading-button'
...
export function HeadingDropdownMenu({
  ...
  const canToggleAnyHeading = React.useCallback((): boolean => {
    ...
    const canToggleHeading = levels.some((level) => editor.can().toggleNode('heading', 'paragraph', { level }))
    const canToggleParagraph = includeParagraph ? editor.can().setParagraph() : false
  ...
  const isParagraphActive = includeParagraph && (editor?.isActive('paragraph') ?? false)
  ...
  const show = React.useMemo(() => {
    ...
    return true
  }, [headingInSchema, paragraphInSchema, editor, hideWhenUnavailable, canToggleAnyHeading])
